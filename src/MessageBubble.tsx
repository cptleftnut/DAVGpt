    ...
    <div className="code-block">
      <div className="code-header">
        ...
        <div className="code-actions">
          <button className="code-btn" onClick={copy} aria-label={copied ? 'Copied' : 'Copy code'}>{copied ? '✓' : 'Copy'}</button>
          {canRun && (
            <button
              className={`code-btn run-btn ${ran ? 'ran' : ''} ${connState !== 'connected' ? 'offline' : ''}`}
              onClick={handleRun}
              aria-label={ran ? 'Sent command' : connState === 'connected' ? 'Run command' : 'Run command (connect first)'}
            >
  ...
  return (
    <div className="bubble ai-bubble">
      <div className="bubble-actions">
        {onSpeak && (
          <button className="speak-btn" onClick={() => speaking ? window.speechSynthesis?.cancel() : onSpeak(content)} title={speaking ? "Stop reading" : "Read aloud"} aria-label={speaking ? "Stop reading" : "Read aloud"}>
            {speaking ? '⏹' : '🔊'}
          </button>
        )}
      </div>
