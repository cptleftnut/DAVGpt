  ...
  return (
    <div className="chat-page">
      <header className="header">
        <button className="menu-btn" aria-label="Open menu" title="Open menu" onClick={onOpenSidebar}>☰</button>
      ...
      <footer className="input-bar">
        <div className="input-wrap">
          {!isHermes && <button className="skill-btn" aria-label="Toggle skills panel" title="Toggle skills panel" onClick={() => setShowSkills(true)} disabled={!apiKey}>⚡</button>}
          {stt.supported && (
            <button className={`mic-btn ${stt.listening ? 'listening' : ''}`} aria-label={stt.listening ? 'Stop recording' : 'Start recording'} title={stt.listening ? 'Stop recording' : 'Start recording'} onClick={stt.listening ? stt.stop : stt.start}>
              {stt.listening ? '⏹' : '🎤'}
            </button>
          )}
