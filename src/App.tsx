  ...
  return (
    <div className="chat-page">
      <header className="header">
        <button className="menu-btn" aria-label="Open sidebar menu" onClick={onOpenSidebar}>☰</button>
        <div className="header-center">
          <span className="session-env-icon">{env.icon}</span>
          <span className="session-name">{session.name}</span>
        </div>
        <div className="header-right">
          <button className="icon-btn" aria-label="Open MCP Servers panel" onClick={onOpenMCP} title="MCP Servers">
      ...
      {activeSkill && <div className="skill-banner">{activeSkill.icon} {activeSkill.label}<button className="skill-clear" aria-label="Clear active skill" onClick={() => { setActiveSkill(null); setInput('') }}>✕</button></div>}
      ...
      <footer className="input-bar">
        <div className="input-wrap">
          {!isHermes && <button className="skill-btn" aria-label="Open skills menu" onClick={() => setShowSkills(true)} disabled={!apiKey}>⚡</button>}
          {stt.supported && (
            <button className={`mic-btn ${stt.listening ? 'listening' : ''}`} aria-label={stt.listening ? "Stop recording" : "Start recording"} onClick={stt.listening ? stt.stop : stt.start}>
              {stt.listening ? '⏹' : '🎤'}
            </button>
          )}
