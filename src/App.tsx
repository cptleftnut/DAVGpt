    ...
    <div className="chat-page">
      ...
      <footer className="input-bar">
        <div className="input-wrap">
          {!isHermes && <button className="skill-btn" onClick={() => setShowSkills(true)} disabled={!apiKey} aria-label="Use Skill" title="Use Skill">⚡</button>}
          {stt.supported && (
            <button className={`mic-btn ${stt.listening ? 'listening' : ''}`} onClick={stt.listening ? stt.stop : stt.start} aria-label={stt.listening ? 'Stop voice input' : 'Start voice input'} title={stt.listening ? 'Stop voice input' : 'Start voice input'}>
              {stt.listening ? '⏹' : '🎤'}
            </button>
          )}
