  ...
  return (
    <div className="chat-page">
      <header className="header">
        <button className="menu-btn" aria-label="Open Sidebar" onClick={onOpenSidebar}>☰</button>
      ...
      <footer className="input-bar">
        <div className="input-wrap">
          {!isHermes && <button className="skill-btn" aria-label="Open Skills" onClick={() => setShowSkills(true)} disabled={!apiKey}>⚡</button>}
          {stt.supported && (
            <button className={`mic-btn ${stt.listening ? 'listening' : ''}`} aria-label={stt.listening ? 'Stop dictation' : 'Start dictation'} onClick={stt.listening ? stt.stop : stt.start}>
              {stt.listening ? '⏹' : '🎤'}
            </button>
          )}
