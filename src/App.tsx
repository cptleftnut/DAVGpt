  ...
  return (
    <div className="chat-page">
      <header className="header">
        <button className="menu-btn" onClick={onOpenSidebar} aria-label="Open sidebar menu">☰</button>
        <div className="header-center">
          <span className="session-env-icon">{env.icon}</span>
          <span className="session-name">{session.name}</span>
        </div>
        <div className="header-right">
          <button className="icon-btn" onClick={onOpenMCP} title="MCP Servers" aria-label="Open MCP Servers">
        ...
        <div className="modal-overlay" onClick={() => apiKey && setShowSettings(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="setting-row">
              ...
              <button className={`toggle-btn ${tts.autoSpeak ? 'on' : ''}`} onClick={tts.toggleAutoSpeak} aria-label={`Toggle auto-speak responses, currently ${tts.autoSpeak ? 'ON' : 'OFF'}`}>{tts.autoSpeak ? 'ON' : 'OFF'}</button>
            </div>
      ...
      <footer className="input-bar">
        <div className="input-wrap">
          {!isHermes && <button className="skill-btn" onClick={() => setShowSkills(true)} disabled={!apiKey} aria-label="Open skills menu">⚡</button>}
          {stt.supported && (
            <button className={`mic-btn ${stt.listening ? 'listening' : ''}`} onClick={stt.listening ? stt.stop : stt.start} aria-label={stt.listening ? "Stop recording" : "Start recording"}>
              {stt.listening ? '⏹' : '🎤'}
            </button>
          )}
